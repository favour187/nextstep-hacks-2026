...
export function Button({ variant = "primary", size = "md", loading = false, disabled, className, children, ...rest }: ButtonProps) {
    return (<button className={cx("btn", `btn-${variant}`, `btn-${size}`, className)} disabled={disabled || loading} {...rest}>
...
export function Card({ className, children, style, }: {
    ...
    return (<div className={cx("card", className)} style={style}>
      {children}
    </div>);
}
...
export function Input({ label, error, hint, id, className, ...rest }: InputProps) {
    ...
    return (<div className={cx("field", className)}>
      {label && (<label className="field-label" htmlFor={inputId}>
          {label}
        </label>)}
      <input id={inputId} className={cx("input", error && "input-error")} aria-invalid={error ? true : undefined} {...rest}/>
...
export function ProgressBar({ value, label, tone = "accent", }: {
    ...
    return (<div className="progress" role="progressbar" aria-valuenow={Math.round(clamped)} aria-valuemin={0} aria-valuemax={100}>
