...
export function StepWiseApp() {
    ...
    if (auth.booting || loading) {
        return (<div className="page" style={{ display: "grid", placeItems: "center", minHeight: "50vh" }}>
        <Spinner size={28}/>
      </div>);
    }
    if (view === "new" || !active) {
    ...
    return (<div>
      {goals.length > 1 && (<div className="row" style={{ gap: 8, paddingTop: 12 }}>
          <span style={{ fontSize: 13, color: "var(--text-2)" }}>Plans:</span>
          {goals.map((g) => (<button key={g.id} className={cx("btn", g.id === active.id ? "btn-primary" : "btn-secondary", "btn-sm")} onClick={() => setActiveId(g.id)}>
              {g.category}
            </button>))}
        </div>)}
    ...
    return (<div>
      ...
      <div style={{ maxWidth: 460, margin: "var(--space-7) auto 0" }}>
        <Card className="stack">
          ...
          <button className="btn btn-ghost" onClick={() => setMode(mode === "login" ? "register" : "login")} style={{ width: "100%" }}>
