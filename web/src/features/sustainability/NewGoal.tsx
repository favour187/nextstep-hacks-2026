...
export function NewGoal({ onCreated, onCancel }: {
    onCreated: () => void;
    onCancel: () => void;
}) {
    ...
    return (<div className="page">
      ...
      <Card className="stack">
        ...
        <Input id="goal" value={goalText} onChange={(e) => setGoalText(e.target.value)} placeholder="e.g. I want to reduce the waste my household produces" maxLength={400}/>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {EXAMPLES.map((ex) => (<button key={ex} type="button" className="btn btn-ghost btn-sm" onClick={() => setGoalText(ex)}>
        ...
        <div className="grid-2">
          <div className="field">
            ...
            <input id="hours" type="range" min={0.5} max={10} step={0.5} value={hours} onChange={(e) => setHours(Number(e.target.value))}/>
          </div>
          <div className="field">
            <label className="field-label" htmlFor="budget">
              Budget you can spend: <strong>${budget}/week</strong>
            </label>
            <input id="budget" type="range" min={0} max={50} step={5} value={budget} onChange={(e) => setBudget(Number(e.target.value))}/>
          </div>
        </div>
        <div className="field">
          ...
          <input id="horizon" type="range" min={7} max={60} step={1} value={horizon} onChange={(e) => setHorizon(Number(e.target.value))}/>
        </div>
      ...
      <Card>
        ...
        ].map(([icon, title, body]) => (<div key={title} className="row" style={{ alignItems: "flex-start" }}>
