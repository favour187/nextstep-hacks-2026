/** Formatting + small presentation helpers for StepWise. */
...
export function effortLabel(score: number): string {
  if (score >= 0.78) return "Very easy";
  if (score >= 0.62) return "Easy";
  if (score >= 0.5) return "Moderate";
  return "Challenging";
}

export const IMPACT_ROWS: Array<{ key: keyof Impact; label: string; icon: string }> = [
...
/** Simple 7-day activity series from check-ins. */
export function weeklyActivity(checkIns: { date: string }[]): { label: string; count: number }[] {
...
/** Streak: consecutive days with ≥1 check-in, ending today (or yesterday). */
export function streakDays(checkIns: { date: string }[]): number {
