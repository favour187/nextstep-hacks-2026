...
import { CATEGORY_LABEL, CATEGORY_TONE, IMPACT_ROWS, effortLabel, fmtImpact, fmtNum, streakDays, weeklyActivity, } from "./format";
    ...
    return (<div className="page">
      ...
      <div className="grid-2">
        <Card className="stack">
          ...
          {IMPACT_ROWS.filter((r) => goal.impact[r.key] > 0).length === 0 ? (<p style={{ color: "var(--text-2)", fontSize: 14, margin: 0 }}>
              No impact recorded yet. Check in below to start the counter.
            </p>) : (IMPACT_ROWS.filter((r) => goal.impact[r.key] > 0).map((r) => (<ImpactMeter key={r.key} label={`${r.icon} ${r.label}`} value={goal.impact[r.key]} max={topImpact}/>)))}
        </Card>
      ...
      <div className="grid-2" style={{ gridTemplateColumns: "minmax(0, 1.1fr) minmax(0, 0.9fr)" }}>
        ...
        <Card className="stack">
          ...
          {plan.feasibility.map((group) => (<div key={group.name} className="stack" style={{ gap: 8 }}>
              <div className="spread">
      ...
      <div className="grid-2">
        <Card className="stack">
          <span className="spread">
            ...
            <Button size="sm" loading={saving} disabled={selected.size === 0} onClick={() => void submitCheckIn()}>
          ...
          {plan.chosen.map((action) => {
            ...
            return (<label key={action.action_id} className={cx("row", "checkin-row", checked && "checkin-row-done")} style={{
                    gap: 10,
                    padding: 10,
                    borderRadius: 10,
                ...
                <input type="checkbox" checked={checked} onChange={() => toggle(action.action_id)} style={{ width: 18, height: 18, accentColor: "var(--accent)" }}/>
                <div style={{ flex: 1 }}>
                ...
                {action.score !== null && (<Badge tone="neutral">{Math.round(action.score * 100)}% fit</Badge>)}
              </label>);
        })}
        </Card>
        ...
        <Card className="stack">
          ...
          {plan.milestones.map((m, i) => (<div key={i} className="row" style={{ gap: 10, alignItems: "flex-start" }}>
              <span className={cx("milestone-dot", m.done && "milestone-dot-done")} style={{
                width: 14,
                height: 14,
                borderRadius: "50%",
