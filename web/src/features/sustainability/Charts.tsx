    ...
    return (<div style={{ display: "grid", gap: 6 }}>
      <div style={{
        ...
        {data.map((d, i) => {
            ...
            return (<div key={i} style={{ flex: 1, display: "grid", gap: 4, justifyItems: "center" }}>
              <div title={`${d.count} check-in${d.count === 1 ? "" : "s"}`} style={{
                    width: "70%",
                    height: h,
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {data.map((d, i) => (<div key={i} style={{ flex: 1, textAlign: "center", fontSize: 10, color: "var(--text-3)" }}>
            {d.label}
          </div>))}
      </div>
    </div>);
}
export function ImpactMeter({ value, max, label }: {
    value: number;
    max: number;
    label: string;
}) {
