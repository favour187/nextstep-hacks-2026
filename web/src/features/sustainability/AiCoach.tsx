...
export function AiCoach({ goalId }: {
    goalId: string | null;
}) {
    ...
    return (<div className="stack" style={{ minHeight: 320, display: "flex", flexDirection: "column" }}>
      <div ref={listRef} style={{
        ...
        {messages.map((m, i) => (<div key={i} className={cx("msg", m.role === "user" ? "msg-user" : "msg-assistant")} style={{
                maxWidth: "85%",
                padding: "10px 14px",
                borderRadius: 14,
                fontSize: 14,
            ...
            {m.text}
            {m.fallback && (<div style={{ marginTop: 6, fontSize: 11, opacity: 0.65 }}>
                <Badge tone="neutral">deterministic demo answer</Badge>
              </div>)}
          </div>))}
        {busy && (<div style={{ justifySelf: "start" }}>
            <Spinner size={16}/>
          </div>)}
      </div>

      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        {SUGGESTIONS.map((s) => (<button key={s} className="btn btn-secondary btn-sm" onClick={() => void send(s)} disabled={busy}>
            {s}
          </button>))}
      </div>
      ...
      <form className="row" style={{ alignItems: "stretch" }} onSubmit={(e) => {
        ...
        <input className="input" style={{ flex: 1 }} value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask about your plan…" aria-label="Chat message"/>
