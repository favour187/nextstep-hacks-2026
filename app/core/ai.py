        ...
        for skill in self.skills:
            if skill.can_handle(user, system):
                try:
                    ...
                    logger.exception(
                        "Local AI skill %s failed; falling back to default", skill.id
                    )
                    continue
...
class AIGateway:
        ...
        *,
        ...
        try:
            ...
            if self.settings.ai_mode == "auto" and self._provider is None:
                logger.warning(
                    "Remote AI unavailable (%s); using local fallback.", exc.code
                )
    ...
    def _cache_key(
        ...
        digest = hashlib.sha256(
            f"{self .provider_name }|{system }|{user }|{temperature }|{max_tokens }".encode(
                "utf-8"
            )
        ).hexdigest()
        return digest
    ...
    def status(self) -> dict[str, Any]:
        backend = (
            "local" if not self.settings.ai_remote_configured else self.settings.ai_mode
        )
        return {
            "mode": self.settings.ai_mode,
            "backend": backend,
