...
def test_full_flow(client):
    ...
    assert all(
        isinstance(f, dict) and "multiplier" in f for f in goal["plan"]["feasibility"]
    )
    assert (
        goal["plan"]["total_impact"]["kg"] > 0
        or goal["plan"]["total_impact"]["items"] > 0
    )
    goal_id = goal["id"]
    ...
    fetched = client.get(
        f"/api/sustainability/goals/{goal_id }", headers=headers
    ).json()
...
def test_goal_requires_meaningful_text(client):
    headers = _auth(client)
    res = client.post(
        "/api/sustainability/goals", headers=headers, json={"goal_text": "hi"}
    )
